...
uint8_t stopTransmitting = 0;

uint8_t isStopTransmitting(void){
  if (stopTransmitting){
    stopTransmitting = 0;
    return 1;
  }
...
int ble_init() {
  ...
  uint8_t bdaddr[] = {0x12, 0x72, 0xe9, 0x66, 0x06, 0x00};
...
void ble_loop() {
  ...
  if (ble_rx_buffer_len) {  //Check if data is available
    if (ble_rx_buffer_len == 1 && ((char*)ble_rx_buffer)[0] == STOP_CHAR)
      stopTransmitting = 1;
